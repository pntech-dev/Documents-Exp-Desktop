...
RELEASE_NOTES = {
    "0.2.9": [
        {
            "title": "Улучшения редактора документов",
            "items": [
                "Редактор документов теперь устойчивее обрабатывает редкие проблемы с данными сессии.",
                "В проблемных ситуациях фоновые операции с документом завершаются безопасно, без срыва сценария.",
                "Повышена общая надёжность сохранения и загрузки файлов после перезапуска приложения.",
            ],
        },
    ],
}
