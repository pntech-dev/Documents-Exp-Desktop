...
RELEASE_NOTES = {
    "0.2.2": [
        {
            "title": "Стабильность поиска и таблицы",
            "items": [
                "Исправлена периодическая проблема, при которой после очистки поиска не возвращались все документы выбранной категории.",
                "Исправлена рассинхронизация состояния, из-за которой после очистки поиска таблица могла оставаться пустой при переключении категорий или отделов.",
                "Улучшена обработка строки поиска, состоящей только из пробелов.",
            ],
        },
    ],
}
