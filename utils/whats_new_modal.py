    ...
    "0.2.0": [
        {
            ...
            "items": [
                "Теперь можно редактировать данные профиля пользователя прямо в приложении: имя, фамилию и отдел.",
                "Диалог профиля доступен из главного меню приложения.",
            ],
        },
        {
            "title": "Сохранение настроек",
            "items": [
                "Приложение теперь запоминает персональные настройки отдельно для каждого пользователя.",
                "Выбранная тема интерфейса сохраняется между сессиями.",
                "Последние использованные фильтры поиска сохраняются и ускоряют повторную работу.",
            ],
        },
        {
            "title": "Также в этом обновлении",
            "items": [
                "Добавлено отдельное окно «Что нового», которое показывается после обновления и отображает список изменений версии.",
                "Небольшие улучшения интерфейса и общие исправления ошибок для более стабильной работы.",
                "Улучшена стабильность после простоя: список документов и результаты поиска больше не исчезают при ошибках повторной загрузки.",
                "Стартовая загрузка главного окна вынесена из UI-потока, поэтому приложение меньше подвисает при открытии.",
                "Сценарии авторизации теперь корректно завершаются ошибкой, если системное хранилище сессии недоступно.",
                "API-клиент теперь корректно обрабатывает пустые успешные ответы, например 204 No Content.",
